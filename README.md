...
**81 MCP tools registered across 3 profiles.** Defaults to `full` so upgrades preserve every tool; set `MCP_TOOLS_PROFILE` in `claude_desktop_config.json` env to opt into the smaller surfaces.

| Profile | Tool count | When to use |
|---|---|---|
| `core` | 26 | Default-recommended: smallest agent context, unified surface only. |
| `experimental` | 81 | Dev / diagnostic surface: every device-namespaced tool + raw MIDI primitives. |
| `full` | 81 | Compatibility baseline; preserves every registered tool. |
