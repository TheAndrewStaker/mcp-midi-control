...
across Fractal AM4, Axe-Fx II XL+, and ASM Hydrasynth Explorer; the
tool surface ships with three profiles (see the inventory table
below), and every wire-level tool ships with byte-exact goldens
against real captures. Axe-Fx II preset authoring is
...
**81 MCP tools registered. The default `core` profile exposes 26 of them** (unified surface essentials + conversational generic-MIDI). Set `MCP_TOOLS_PROFILE=experimental` or `=full` in `claude_desktop_config.json` env to expose the larger surfaces (device-namespaced tools, raw MIDI primitives, diagnostics).

| Profile | Tool count | When to use |
|---|---|---|
| `core` | 26 | Default. Smallest agent context; daily driver for tone-building and preset work. |
| `experimental` | 79 | Hardware-specific control + diagnostic probes; raw MIDI send_* primitives. |
| `full` | 81 | Every registered tool. Use if you need a wrapper the smaller profiles hide. |

Full per-profile tool list with description-length stats: [`docs/TOOLS.md`](docs/TOOLS.md). Generated by `npm run tools:inventory`; preflight checks for drift.
...
- **Name and save presets.** *"Save this to Z4 and call it 'Clean
  Machine'."*
...
The server ships three tool profiles selected at boot via the `MCP_TOOLS_PROFILE` env var. The default is `core` (26 conversational essentials); device-namespaced tools, raw-MIDI primitives, and diagnostics live in the larger profiles.

| Profile | Tools | When to use |
|---|---|---|
| `core` (default) | ~26 unified-surface essentials + conversational generic-MIDI | Smallest agent context. Daily driver for tone-building, preset audition/save/switch, scene control, lineage research. |
| `experimental` | All tools except a small reserved debug set | Hardware-specific control + diagnostic probes (Axe-Fx II grid routing, III tempo/looper/tuner, Hydra patch dump, AM4 state reads, raw send_*). |
| `full` | Every registered tool | Compatibility baseline matching the v0.1 surface. Use if you depend on a tool the smaller profiles hide. |

To override the default, add `env` to your Claude Desktop config:
...
```json
{
  "mcpServers": {
    "mcp-midi-control": {
      "command": "node",
      "args": ["C:\\path\\to\\mcp-midi-control\\packages\\server-all\\dist\\server\\index.js"],
      "env": { "MCP_TOOLS_PROFILE": "full" }
    }
  }
}
```

At startup, the server prints the active profile and the number of hidden tools to the MCP log panel (`Tool profile: core (55 tools hidden — set MCP_TOOLS_PROFILE=full in claude_desktop_config.json env to disable filtering)`). Unknown profile values fall back to `core` with a stderr warning, so a typo in the config never bricks the server.
...
| Tool | What it does |
|---|---|
| `am4_get_active_location` | Read the currently-loaded preset location (A1-Z4). |
