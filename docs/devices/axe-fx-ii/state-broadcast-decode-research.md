# Axe-Fx II 0x74/0x75/0x76 preset-dump triple — initial decode

**Source captures (gitignored):**
- `samples/captured/session-58-axefx2-grid-move.syx` (HW-082 equivalent)
- `samples/captured/session-58-axefx2-block-add.syx` (HW-083 equivalent)
- `samples/captured/session-58-axefx2-knob-turn.syx`

Decoded 2026-05-11 from the passive-capture haul produced by HW-085
Plan F (`scripts/capture-midi-passive.ts`). Triple appears ONLY in
write-action captures (zero in `session-58-axefx2-direct-sync.syx`
which was read-only). **Almost certainly the wire-level preset-state
dump envelope for Axe-Fx II XL+ (Q8.02).**
...
## Semantic mapping — first results (Session 60, 2026-05-11)
...
in positions 117..235 that look like a second concatenated state —
possibly the per-channel state for another channel, or modifier
...
1. **Grid-cell state dump**: 4 rows × 12 cols = 48 cells. If each cell
   carries ~3 attributes (block-id, channel?, bypass?), that's 144 —
   close to 140 with some cells being empty / collapsed.
...
- **BK-048 pilot decode target ✅** — first capture-derived editor-
