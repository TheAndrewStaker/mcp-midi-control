...
Use this when you want to capture what a vendor editor sends to your device while it connects and syncs. The Fractal editors (Axe-Edit III, FM9-Edit, VP4-Edit, AM4-Edit) are the worked examples below, but the same steps apply to any CoreMIDI device: MIDI Control Center for an Arturia synth, Novation Components for a Circuit, and so on. This is the macOS equivalent of the [USBPcap and Wireshark](usbpcap-wireshark.md) guide used on Windows.

**Raise "Remember up to N events" in the main toolbar before you start.** The default of 1000 is the single biggest cause of an unusable capture: an editor's read-poll can flush your edits out of the buffer before you save.
...
## Pairing with your device's asks

If you have already completed the SESSION-tier asks on
[your device's page](../README.md), your device and the server are confirmed
working. A capture session is a separate step and does **not** need the MCP
server running: just your device and its editor. In fact the server must be
quit, because it would hold the port the editor needs.

The full ask descriptions, including exactly what to do while recording and what
to write down alongside, live on the device page itself.
