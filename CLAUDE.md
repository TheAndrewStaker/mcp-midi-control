...
## Current Phase
**Status:** pre-release. AM4 + Hydrasynth functional; Axe-Fx II functional; the modern Fractal family (Axe-Fx III / FM3 / FM9) in community beta via one shared gen-3 codec factory; the FM3's core surface (USB-serial transport, reads, continuous param writes, bypass, scenes, preset switching) is hardware-CONFIRMED end-to-end through this server's own probes by a 2026-06-12 community field test, and set-by-name discrete param writes are FM3-hardware-confirmed via a 2026-06-10 collaborator session (frames byte-identical to our encoder, sent from the tester's rig). A 2026-06-27 field test (ForgeFX, an independent app built on this codec, real FM3 over Linux USB-serial) then hardware-confirmed `set_block` placement (the FM3 needs an extra cell-select frame ahead of the insert, which this server already sends), grid routing (cable connect/disconnect), and the Linux leg of the serial transport; `save_preset` and the Windows serial-driver path still await on-device confirmation. The **FM9** is now also community-confirmed for the read + continuous-write path: a 2026-06-17 owner test (fw 11.0 / macOS) round-tripped `get_param` + continuous `set_param` on hardware through this server (acked, values confirmed on the FM9-Editor display), plus channel-specific reads and alias resolution. A 2026-06-19 Windows verify probe then hardware-confirmed `set_block` placement (a placed block appeared in the device's own fn=0x13 status dump) and `switch_scene` round-trip on the FM9, and reading a block's current type/model by NAME is wire-confirmed (fn=0x01 sub=0x1F → the device's own name string); discrete set-by-name, `save_preset`, and the live grid read (fn=0x01 sub=0x2E) stay community-beta on the FM9. The **Axe-Fx III** (the gen-3 byte-identity anchor) is now also hardware-confirmed end-to-end for the first time: the same 2026-06-17 owner test ran `set_param` (amp gain, channel A) with a device echo and a `get_param` read-back matching the front panel (same beta carve-outs as the FM9). A 2026-06-18 community roundtrip then exercised SET→GET across the entire FM9 and III catalogs on hardware: it confirmed the read + continuous-write paths catalog-wide and surfaced one fixed catalog bug: FM9 enum/type selectors were routing as continuous floats instead of discrete ordinals (~351 params corrected, gated on the FM9's own editor-cache enum data, so only the FM9 changed and the III byte-identity is intact). The III then got the same correction on 2026-06-20: the 2026-06-18 III roundtrip is itself the device oracle (the wire values it returned ARE the device's quantized image), so ~92 III type/model/count selectors that had been routing as continuous floats are now routed discrete; byte-identity intact (no wire builder changed, only kind-classification at the catalog boundary). The earlier framing that III/FM3/VP4 "need a fresh device-synced editor cache, the copies on hand are unsynced stubs" was too strong: the on-disk editor cache IS an enum-less stub, but a SET→GET roundtrip is an equally valid oracle and unblocks the fix with data in hand (FM3/VP4 simply lack a roundtrip capture so far). That roundtrip is device-behavior evidence from an independent rig; this server's own discrete write frame stays community-beta. The original Axe-Fx Standard/Ultra (gen-1, model 0x01) in community beta, with parameter set + read decoded from the published gen-1 SysEx spec. See ROADMAP.md.
...
- `@julusian/midi` for USB MIDI (an API-compatible node-midi fork that ships N-API prebuilt binaries for every platform we target: darwin-arm64/x64, win32-x64/arm64/ia32, linux). N-API is ABI-stable, so the prebuilt loads regardless of Node major; a toolchain (VS Build Tools / Xcode CLT) is needed only as a compile fallback when no prebuild matches the platform. End users get a release ZIP with bundled Node + the prebuilt binary, no toolchain. (Swapped from `midi`/justinlatimer-node-midi, which shipped no prebuilds and compiled on every install; the prebuild swap is what unblocks the macOS `.mcpb` Desktop Extension and a no-Xcode Mac install.)
...
| **`@mcp-midi-control/fractal-gen1`** | `packages/fractal-gen1/` | Axe-Fx Standard/Ultra (gen-1) device descriptor. Its **own** nibble-split codec (model `0x01`, fn `0x02`, trailing query(0)/set(1) flag, not gen-2 septet, not gen-3 sub-action), in `fractal-midi/gen1`. **Set + parameter read, community beta**: decoded byte-exactly from the published gen-1 SysEx spec (no hardware). `get_param`/`get_params` query via fn 0x02 (flag 0) → MIDI_PARAM_VALUE (value + device label); `get_preset` ships the patch dump's spec-pinned subset (fn 0x03→0x04: name + effect-grid layout + source flag; param region unpinned → byte count only; bank-C ≥256 refuses); save/switch/scene/channel omitted. 922 params / 35 blocks generated from the doc. |
...
| **`@mcp-midi-control/arturia`** | `packages/arturia/` | Arturia **Freak family**: brand-level SysEx codec (`00 20 6B` + a device-code byte) plus `createFreakDescriptor(config)` and per-device configs in `devices/`. **MicroFreak** is hardware-confirmed (fw 5.0.0): SysEx preset-name read + full 146-packet dump, globals read `0x43` AND write `0x42`, 0-based Program Change, CC from Arturia's Appendix D. **MiniFreak** is `generic-only`: CC/PC with a curated map, and NO SysEx at all because its device-code byte is unknown, so its globals block does not exist and every `get_param` refuses. The **CC table is per-device and must never be shared**: the two models' CC numbers collide with different meanings (cutoff 23 vs 74, resonance 83 vs 71, CC 24 = Cycling-Env Amount vs VCF Env Amount), so a shared table would silently move the wrong parameter. Preset params are WRITE-ONLY on every Freak (the device reports nothing back and its display does not show incoming CC, contrary to the manual); only `system.*` globals read back. |
| **`@mcp-midi-control/circuit-tracks`** | `packages/circuit-tracks/` | Novation Circuit Tracks: synth/drum control + pattern target + `.ncs` project / WAV sample transfer (SysEx file-transfer over USB MIDI). |
| **`@mcp-midi-control/spd-sx`** | `packages/spd-sx/` | Roland SPD-SX. **Two transports:** a MIDI descriptor (kit recall via `switch_preset` + pad triggers, AUDIO/MIDI mode) AND a **storage-transport** tool family (`spdsx_*`, WAVE MGR mode) that reads/authors the device's kits + wave pool over the mounted `Roland/SPD-SX` drive. Storage ops are filesystem, NOT MIDI: `codec/` (kit/WvPrm XML + canonical WAV) + `storage/` (drive discovery, append-only wave import, kit author, backup), ported byte-exact from the hardware-confirmed `scripts/spdsx/*.py`. Building a kit is two steps: `spdsx_import_waves` → `spdsx_author_kit`. See `docs/_private/STATE-SPDSX.md`. |
...
Protocol docs (`SYSEX-MAP.md`, opcode tables, Ghidra mining scripts, encoding-primitive corpus, and the Fractal-editor RE methods under `capture-guides/`) live in `packages/fractal-midi/docs/`; see per-device pointers in "External References" below.

**Contribution guidance is product-repo-owned, in `docs/contributing/`.** Anything that says "install this server", "run `npm run ...`", "open an MCP host" or "file an issue here" belongs there, never in the npm-published codec package, whose consumers are developers building on the wire format. One page per registered device at `docs/contributing/devices/<descriptor-id>.md`, where the filename equals the descriptor `id` so coverage is a mechanical set comparison. `scripts/verify-contribution-guides.ts` (in `preflight`) fails on a missing page, an orphan page, a page whose declared tier / transport / preset_class disagrees with its descriptor, a dead link, or a cited command that does not resolve.
...
### Grid devices (Axe-Fx II / III / FM3 / FM9): grid, shunts, and how to COUNT blocks

The AM4 (and VP4) chain is a serial 4-slot line. The larger Fractal units lay
blocks on a **grid** instead. When wording anything about a grid device (tool
descriptions, `chain_integrity` / `describe_device` output, agent guidance,
docs), use these exact words. **When II and III phrasing differ, lean to the
III's wording** (it is Fractal's current, and it is what generalizes across the
whole modern family):

| Term | What it means |
|---|---|
| **Grid** | The layout matrix a preset's blocks sit on. 12 columns × 4 rows on Axe-Fx II / III / FM3 / FM9. |
| **Grid location** | One cell of the grid `(row, col)`. Fractal also says "**slot**" for a grid position (Axe-Fx II usage); prefer "grid location" when a bare "slot" could be read as the AM4 1-4 effect-slot. |
| **Block** | ANY component placed in a grid location. Fractal groups Amp, Cab, EQ, and every effect all as "blocks" (III Owner's Manual: "another block like an Amp, Cab, or effect"). This is the word for a total placed-block count. |
| **Effect block** | A NARROWER subset: an effect block excludes Amp / Cab / EQ. Do NOT use "effect block" for a total count; it undercounts. Use it only when you specifically mean the effects, not the amp/cab/eq. |
| **Shunt** | A passive connector / "sonically transparent" block (III), a signal pass-through wire that fills a grid location with no processing. NOT a content block; a full row always fills its remaining cells with shunts. |

**Counting rule (the decision that drove the `chain_integrity` wording).** When
you report how many things are on the grid, count **blocks** (content) and call
out **shunts** separately; never fold shunts into the block count, and never
report the raw placed-*cell* count as a block count (a filled row is 12 cells =
content blocks + connecting shunts). The correct shape is
`"intact across N blocks (+M shunts)"`, not `"12 placed cells"`. Misreading the
cell count as a block count is exactly the BUG-8 misdiagnosis (a 5-block preset
read as "12 blocks"); the count MUST split content blocks from shunts.

Anti-patterns (grid): "12 blocks" for a row that is 5 content blocks + 7 shunts
(count content only); "effect block" as a synonym for "block" in a total (it
excludes amp/cab/eq); a bare "slot" on a grid device where it reads as the AM4
1-4 effect slot (say "grid location").
...
**One surface. The device-namespaced tier is gone.**

The live roster is **55 unified tools**, all port-dispatched and device-agnostic, registered from `packages/core/src/protocol-generic/tools/*.ts` (one file per family: `discovery`, `params`, `preset`, `layout`, `navigation`, `patterns`, `uploads`, `backup`, `audio`) and dispatched through `packages/core/src/protocol-generic/dispatcher/`. Adding a device means writing a schema descriptor + wire adapter; no new tools. `docs/TOOLS.md` is the generated inventory, and `npm run tools:inventory-check` fails on drift.

The old `am4_*` / `axefx2_*` / `hydra_*` tier was removed: its device-specific behavioural guidance now rides `describe_device`, which is where it belongs, because it is paid only by a session that asks for that device. One namespaced tool remains in the tree and is deliberately NOT registered (`hydra_navigate_to`, kept as a one-off diagnostic with its registration commented out in `packages/hydrasynth/src/server.ts`); `packages/hydrasynth/src/tools/` otherwise registers unified tools. Do not re-add a namespaced tool.

**A new capability goes on the unified surface or not at all.** If it does not fit, design the contract change first (extend `DeviceWriter` / `DeviceReader` / capabilities), then register the unified tool.

**Every tool's `tools/list` cost is frozen per tool.** `tools/list` is paid on EVERY session before the first tool call, so it is the most expensive text in the product. `npm run verify-tools-list-budget` measures the WHOLE per-tool payload (description + every argument description + schema), not just `.description`, and fails on a per-tool or total ceiling. **Moving prose from a tool description into an argument description does not reduce it; both ship in the same response.** Pay for new argument prose by cutting text the response or an error message already carries, or state the reason and re-freeze the ceiling. Do not re-freeze to make a run go green.

**Every read tool's RESPONSE size is gated too.** See "Response ceilings" below.
...
## Response ceilings, the failure that is invisible from inside

**A tool result over 50,000 characters is not delivered to the model at all.** The host substitutes a short preview plus a path to a sidecar file, and an MCP-only agent (Claude Desktop, and this repo's own `claude -p --tools ""` harness) has no filesystem tool with which to open it. No error, no warning: the model answers from a stub and nothing in the transcript says so. The figure is measured, not inferred, and bisected to a single character (`scripts/probe-host-delivery-cliff.ts`): 50,000 delivered whole, 50,001 not. **Characters, not tokens.**

This has now caused three separate silent outages, each time because the previous fix was read as being about the one tool that got caught: `describe_device`, then `list_params`, then a census that found four more (`lookup_lineage` forward at 219,171 chars, `list_backups` at 212,623, `get_params` at 208,649, `import_songsterr` at 72,927).

**The ceiling belongs to EVERY response whose length is driven by data volume rather than by design.** Before shipping a read tool, measure it at its widest legal arguments and gate it:

| gate | covers |
|---|---|
| `verify-describe-device-budget` | `describe_device` (budgets at exactly 50,000: every character back costs a named guidance topic) |
| `verify-list-params-budget` | `list_params`, all three scopes + the pager |
| `verify-response-budget` | `lookup_lineage`, `list_backups`, `get_params`, and the measured-not-gated rest |

Three rules that keep being relearned:

1. **Narrowing correctly is not a defence.** The AM4 exceeded the cliff on a properly-filtered `list_params({block:["amp"]})`, and `list_backups` measured *larger* with a `device` filter than without at the same limit. Gate the filtered shapes too.
2. **Fit and page; never truncate silently.** Rows are fitted to a 40,000-char budget (not 50,000: a page boundary costs one cheap follow-up call, so take the margin) and the remainder is reported with the argument that fetches it. **A response that was shortened must say so.** A blackout is at least visibly broken; a silently-cut list has the agent telling the user it read the whole roster. Gate the pager as well as the size: one that stalls or drops rows is worse than none.
3. **When a batch costs wire time, refuse up front instead of paging.** Fitting a 894-read batch into the ~240 that fit means performing 654 round-trips and discarding them. `get_params` caps `queries` at 100 and refuses before the first frame goes out (`GET_PARAMS_MAX_QUERIES`), which is also the tighter of the two limits above: the response budget allows ~215-240, the > 5 s wire ceiling allows ~100.

## Key Constraints
- Windows ThinkPad. Use Windows paths.
- `@julusian/midi` normally installs a prebuilt N-API binary (no toolchain). It only falls back to a node-gyp compile when no prebuild matches the platform; if that fallback runs and fails on Windows, install VS Build Tools and `npm rebuild @julusian/midi`.
...
- **`npm run preflight`** is the single command to run before every commit. Runs: `build` (all 15 workspace packages in dependency order), `typecheck` (per-package tsconfigs), `test` (fractal-midi + codec + cross-device + per-device + server smoke), `verify-no-internal-refs`, `primitives-verify`, `tools:inventory-check`, the four size gates (`verify-describe-device-budget`, `verify-list-params-budget`, `verify-response-budget`, `verify-tools-list-budget`), and `verify-agent-harness`.
- **A gate that is not in `preflight` does not exist.** `verify-response-budget.ts` was written, committed, and referenced by nothing for a full release cycle: it measured four tools past the delivery cliff and no one ran it. When you add a gate, register it in `package.json` AND append it to `preflight` in the same commit.
...
| **`docs/contributing/devices/<id>.md`** | Any change to what that device can do or what it needs. A capability ships, moves status word, or is gated; evidence lands from a contributor; an ask closes or is added; the descriptor's `support_tier` or `verification` changes. Same session, no exceptions: a device page that lags its descriptor fails preflight (`scripts/verify-contribution-guides.ts`). The filename IS the descriptor id, so coverage is a set comparison |
| **`docs/contributing/README.md`** | A device is registered or removed; a device's support tier changes; a device's top-ranked ask changes. The device index table must carry exactly one row per registered descriptor, and its tier column is checked against the descriptor |
| **A descriptor's `capabilities.support_tier` / `verification`** | Hardware evidence lands, or a capability is gated or un-gated. **Every registered descriptor must declare `support_tier` explicitly**; relying on the dispatcher default is a preflight failure. A tier that contradicts its own `verification` prose raises a preflight warning for the maintainer to resolve |
| **`docs/contributing/{TIERS,SAFETY,EVIDENCE,SUBMITTING}.md` and `tools/`** | The contribution ladder, the port-exclusivity rule, the evidence vocabulary, or a capture tool's workflow changes. `SAFETY.md` is the CANONICAL port-exclusivity statement; other docs link it rather than restating it |
| **Editor RE methods** in `packages/fractal-midi/docs/capture-guides/` | A Fractal-editor reverse-engineering technique changes. This directory is codec-domain only now; contributor guidance lives in `docs/contributing/` and the old location is a redirect stub the gate keeps small |
