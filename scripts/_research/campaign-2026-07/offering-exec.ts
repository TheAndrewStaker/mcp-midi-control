/**
 * OFFERING populate — DEVICE EXECUTION, 2026-07-31.
 *
 * Give The Offering (Pack 5, Projects 57-63) the condensed four-voice copy of
 * its own MIDI 2 drum part on the Circuit's internal drum tracks: bound
 * [1,2,5,11], stored SILENT (all six levels 0).
 *
 * METHOD (see offering-stage.ts for the full rationale): each project is
 ...
 * chain, a scene table or the project scale. The midi2 leg — including the three
 * `voice_notes`-pinned SPD-SX fills (60/61/62, stored 72/73/74) — is preserved
 * BY CONSTRUCTION rather than reconstructed.
 *
 * FINDING THAT SHAPED THIS RUN: the 2026-07-23 scene-chain conversion and its
 * offset-721 stale-chain fix are NOT on the card. Every capture from 2026-07-30
 * onward, and the card today, is byte-identical to the 2026-07-23 16:59
 * PRE-conversion backup apart from the two synth level bytes (proved by
 * offering-restore-proof.ts). All seven are plain-chain projects, all four scene
 * blocks undefined, 0x2c1 = 0. So there are no scene tables to reproduce, and
 * this run writes none: the plain-chain layout touches only the drum chain slots.
 *
 * Phases (CLI arg):
 ...
 *             tracks written are DRUM ONLY (this is the assertion that proves
 *             no note track is touched), plain chain 1..N matching the
 *             canonical's own midi2 chain, name / colour / bpm unchanged.
 ...
 *
 * Receipts land in samples/_scratch/offering-exec-<phase>.log.json.
 * Run: npx tsx samples/_scratch/offering-exec.ts <phase>
 */
...
export const TARGET_SLOTS = [57, 58, 59, 60, 61, 62, 63];
/** Untouched neighbours: the two Sugar projects nearest the block, both written earlier today and NOT in this run. */
export const WITNESSES = [52, 53];
/** Contiguous bands covering targets + witnesses (backup_device sweeps a range). */
const BANDS: Array<[number, number]> = [[52, 53], [57, 63]];
const PREAUTHOR_DIR = `${ROOT}/samples/circuit-ncs/offering-preauthor-2026-07-31`;
/** Newest canonical per slot, oldest dir first so later dirs win. */
export const CANON_DIRS = [
...
export interface StagedProject {
  ...
  sections: StagedSection[]; order: string[]; pins: string[]; note: string;
}
const STAGED: StagedProject[] = JSON.parse(readFileSync(`${ROOT}/samples/_scratch/offering-staged.json`, 'utf8')) as StagedProject[];
...
if (!phase) { console.error('usage: offering-exec.ts <dryrun|gate|write|backup|scan>'); process.exit(2); }
...
/** Newest canonical per slot across CANON_DIRS (later dirs win on equal slot). */
export function canonicalBySlot(): Map<number, Buffer> {
  const out = new Map<number, Buffer>();
  for (const d of CANON_DIRS) for (const [s, b] of loadBackupDir(d)) out.set(s, b);
  return out;
}
...
async function main(): Promise<void> {
  ...
  const c = new Client({ name: 'offering-exec', version: '1' }, { capabilities: {} });
  await c.connect(t);
  ...
  try {
    if (phase === 'dryrun') {
      const probe = await call('arg-liveness', 'apply_pattern', {
        port: PORT, mode: 'ncs_upload', pack: PACK, ncs_slot: 57,
        ncs_template: 'samples/circuit-tracks/blank_slot20.ncs',
        project_name: 'OfferingArgLivenessProbe0123456789', bpm: 135, dry_run: true,
        voices: { kick: 'x...x...' },
      });
      ...
      for (const tg of STAGED) {
        ...
        // The tool returns JSON; read the receipt PROSE, not its JSON escaping,
        // so a quoted project name matches literally.
        let x: string;
        ...
        const checks: Array<[string, boolean]> = [
          ...
          // track in this list would mean midi2 (and its three pinned SPD-SX
          // fills) is being re-authored.
          ['at least one drum track written', /\bDrum[1-4]\b/.test(trackLine)],
          ['NO note track written (midi2 + its pins preserved)', !/\b(midi1|midi2|synth1|synth2)\b/.test(trackLine)],
          // Negative assertions run against the WHOLE tool payload (warnings
          // included), not just the info prose, so nothing can hide in a warning.
      ...
      const gate = await call('backup-preauthor-gate', 'backup_device', {
        port: PORT, scope: 'stored', from: 57, to: 63, pack: PACK,
        directory: 'samples/circuit-ncs/offering-preauthor-2026-07-31',
      }, 60_000);
      ...
      for (const [from, to] of BANDS) {
        const swp = await call(`backup-preauthor-${from}-${to}`, 'backup_device', {
          ...
          directory: 'samples/circuit-ncs/offering-preauthor-2026-07-31', acknowledge_duration: true,
        }, 600_000);
        if (swp.err) { fail(`preauthor sweep ${from}-${to} ERROR: ${swp.text.slice(0, 400)}`); return; }
      }
      const pre = loadBackupDir(PREAUTHOR_DIR);
      const canon = canonicalBySlot();
      let identical = 0;
      ...
      const gate = await call('backup-authored-gate', 'backup_device', {
        port: PORT, scope: 'stored', from: 57, to: 63, pack: PACK,
        directory: 'samples/circuit-ncs/offering-authored-2026-07-31',
      }, 60_000);
      ...
      for (const [from, to] of BANDS) {
        const swp = await call(`backup-authored-${from}-${to}`, 'backup_device', {
          port: PORT, scope: 'stored', from, to, pack: PACK,
          directory: 'samples/circuit-ncs/offering-authored-2026-07-31', acknowledge_duration: true,
        }, 600_000);
      ...
      for (let i = 1; i <= 2; i++) {
        ...
        console.log(`  offering names: ${names.join(', ')}`);
    ...
    writeFileSync(`${ROOT}/samples/_scratch/offering-exec-${phase}.log.json`, JSON.stringify(log, null, 2));
    await c.close();
  }
