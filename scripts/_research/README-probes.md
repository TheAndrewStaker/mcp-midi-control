# Axe-Fx II probe suite (Session 103)
...
After running all three, the BK-070 atomic-read primitive resolves to
one of:
...
| fn 0x0E returns a richer payload with an empty query | Re-decode it (the session-58 capture had a 54-byte response that may be block inventory + state combined) | Look at the new bytes; might supersede fn 0x1F. |
| None of the above respond | Each is fire-and-forget; the device doesn't expose a bulk-read primitive in any of these envelopes | BK-070 path is back to scene-walk OR preset-binary decode (the harder option). |
...
in `fractal-midi` + `mcp-midi-tools`.
