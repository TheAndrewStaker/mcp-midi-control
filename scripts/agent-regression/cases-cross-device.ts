 ...
 *
 * Cases that exercise the `translate_preset` tool — the marquee cross-device
 ...
 * that re-issued translate_preset 2-3 times because the cross-device alias
...
export const CROSS_DEVICE_CASES: AgentRegressionCase[] = [
  // translate_preset — AM4 source → Axe-Fx II target, dry-run mode.
  //
  ...
  // translate_preset once, and the result should carry a non-empty
  // applied_spec + zero `validation_errors`. The bouncing metric:
  // exactly ONE translate_preset call. A retry signals the agent didn\'t
  ...
  // source-spec vocabulary — that\'s the regression.
  {
    ...
    device: 'am4',
    tier: 'no-hardware',
    description: 'translate_preset (AM4 → Axe-Fx II, dry-run) — Wave 1 cross-device alias + concept-key resolver should let the agent port a preset on the FIRST tool call. Bouncing-regression: catches an agent that retries translate_preset 2-3 times because it second-guessed the alias/enum substitutions.',
    ...
    expectations: {
      ...
      max_tools: 6,
      // The single most important assertion: exactly ONE translate_preset
      // call. Retries = bouncing.
      max_repeats: { translate_preset: 1 },
      tool_call_validators: [{
        ...
        check: (args, result) => {
          ...
          if (!src.includes('am4')) {
            return `translate_preset source_port should be "am4", got ${String(args.source_port)}.`;
          }
          if (!tgt.includes('axe-fx-ii') && !tgt.includes('axefx2') && !tgt.includes('axe-fx ii')) {
            return `translate_preset target_port should be "axe-fx-ii", got ${String(args.target_port)}.`;
          }
          ...
          if (dryRun !== true && targetLoc !== undefined) {
            return `translate_preset should run in dry-run mode (the prompt says "without firing any writes") — got dry_run=${JSON.stringify(dryRun)} target_location=${JSON.stringify(targetLoc)}.`;
          }
          // Result envelope must surface the translated spec, not just
          // bounce with validation errors.
          if (result === undefined) {
            return `translate_preset returned no result body.`;
          }
          if (/"ok"\s*:\s*false/i.test(result) && !/applied_spec/i.test(result)) {
            return `translate_preset returned ok:false with no applied_spec — translator failed instead of cross-aliasing. Result snippet: ${result.slice(0, 280)}.`;
          }
          return true;
        },
      }],
