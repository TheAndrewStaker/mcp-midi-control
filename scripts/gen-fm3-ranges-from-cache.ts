/**
 * Generate `packages/fractal-midi/src/gen3/fm3/ranges.generated.ts`: the
 * device-true FM3 display-range dictionary (min/max/scale/step/typecode plus
 * enum counts) per (block family, paramId), mined from an FM3-Edit
 ...
 *        samples/fm3/effectDefinitions_11_12p0.cache --verify
 *   2. npx tsx scripts/gen-fm3-ranges-from-cache.ts \
 *        [samples/fm3/effectDefinitions_11_12p0.walk.json]
 *
 ...
 *     0xfff0..0xfff3 cab-IR tables), which the wire stride excludes: the gen-3
 *     sub=0x01 block-definition response (FM9-confirmed; FM3 cache matches)
 *     reports CABINET paramCount 106 while the cache CABINET section declares
 *     110 records (4 cab tables). The raw
 ...
 * OUTPUT (the generated .ts) is committed. Gate: scripts/verify-fm3-ranges.ts.
 */
import { readFileSync, writeFileSync } from 'node:fs';
import { resolve, dirname } from 'node:path';
import { fileURLToPath } from 'node:url';
import { FM3_PARAMS_BY_FAMILY } from '../packages/fractal-midi/src/gen3/fm3/params.js';

const __dirname = dirname(fileURLToPath(import.meta.url));
const root = resolve(__dirname, '..');

const WALK = process.argv[2]
  ?? resolve(root, 'samples/fm3/effectDefinitions_11_12p0.walk.json');
const OUT = resolve(root, 'packages/fractal-midi/src/gen3/fm3/ranges.generated.ts');
 ...
 *  (gen-3 sub=0x01 cross-validation is FM9-confirmed; the FM3 cache CABINET is
 *  likewise 106 ordinary vs 110 declared). bySection is filtered to ordinary records. */
...
/** Evidence-anchored section tags — the gen-3 block-family tags are shared across the
 *  family; FM3 anchors confirmed against the device-synced FM3-Edit cache. */
const SEEDS: Readonly<Record<string, number>> = {
  DISTORT: 10, // amp; FM3 331-enum at id 6
  CABINET: 11, // FM3 45-cab roster + cab-IR tables
  REVERB: 12, // FM3 79-enum at id 0, REVERB_TIME 0.1..100 step 0.02
  DELAY: 13,
  FUZZ: 25, // drive; FM3 86-enum at id 0
};
...
function score(family: string, sec: Map<number, WalkRecord>): number {
  let s = 0;
  for (const p of FM3_PARAMS_BY_FAMILY[family] ?? []) {
...
const families = Object.keys(FM3_PARAMS_BY_FAMILY);
...
for (const { family, tag, score: s } of pairScores) {
  ...
  const famSize = (FM3_PARAMS_BY_FAMILY[family] ?? []).length;
...
for (const family of families) {
  for (const p of FM3_PARAMS_BY_FAMILY[family]) nameOf.set(`${family}/${p.paramId}`, p.name);
}
...
const banner = `// GENERATED by scripts/gen-fm3-ranges-from-cache.ts. DO NOT EDIT BY HAND.
// Source: FM3-Edit effectDefinitions cache (FM3 firmware 12.0, device-synced
// 2026), decoded by the strict count-driven walker
...
// cross-validation is FM9-confirmed (CABINET 106 vs 110); the FM3 cache shows
...
// NOT yet wired into live catalog resolution; gate: scripts/verify-fm3-ranges.ts.
//
// Regenerate:
//   npx tsx scripts/_research/parse-effectdefinitions-cache.ts \\
//     samples/fm3/effectDefinitions_11_12p0.cache --verify
//   npx tsx scripts/gen-fm3-ranges-from-cache.ts
/* eslint-disable */
`;
...
/** Device-true FM3 display ranges, keyed by catalog family then paramId. ${rows} rows. */
