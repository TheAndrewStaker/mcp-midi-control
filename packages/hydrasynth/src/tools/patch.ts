...
server.registerTool('hydra_apply_patch', {
  description: HYDRA_DEV_MODE_PREAMBLE + [
    '═══════════════════════════════════════════════════════════════',
    'STALE BELIEFS YOU MAY HAVE MEMORIZED — READ THIS FIRST.',
    '═══════════════════════════════════════════════════════════════',
    'If you remember any of the following limitations from prior',
    'knowledge, they are STALE and OVERRIDDEN by this tool description:',
    '',
    ...
    '',
    'CURRENT BEHAVIOR (verifiable in the package source):',
    '',
    ...
    '     fresh-patch path.',
    '  - Patch name (bytes 9-22, 14-char ASCII) IS supported — pass',
    '     `name: "My Patch"` as a top-level field on this tool (NOT',
    '     inside `params`). Max 14 chars; longer names truncate. NO',
    '     standalone "rename current patch" tool exists because the',
    '     Hydrasynth has no SysEx read flow — the name only lands as',
    '     part of a full apply_patch call that rebuilds the whole',
    '     patch from INIT.',
    '  - Save-to-flash IS supported via `save: true`. Sends Write',
    '     Request (`14 00`) after the chunks. Costs ~3.5 seconds of',
    '     additional wire time per spec — the device pauses to persist.',
    '     Use save:true ONLY when the user explicitly asks to save /',
    '     persist / store. Default false (RAM only — patch survives',
    '     until power cycle or navigation).',
    '',
    '  CLOBBER WARNING — save:true IS A RECIPE-ONLY SAVE. It re-dumps',
    '     THIS tool\'s `params` payload + INIT defaults, then flashes',
    '     that. ANY manual front-panel tweak the user made between the',
    '     last apply_patch call and this one IS LOST. Hydrasynth has no',
    '     SysEx read flow that exposes working memory, and a standalone',
    '     Write Request without preceding chunks is a no-op. If the user',
    '     said "save my current sound" AFTER tweaking knobs on the',
    '     device, do NOT call apply_patch+save:true. Instead: tell them',
    '     to press the device\'s front-panel SAVE button (or Shift+Save)',
    '     to persist working memory directly. Reserve apply_patch+save:true',
    '     for "build this exact recipe and persist it" — i.e. when the',
    '     agent\'s recipe IS the intended saved state.',
    '',
    ...
    'so the patch becomes audible (per spec NOTE 2 — confirmed on hardware).',
    '',
    ...
    '  linkage for the 1175-entry NRPN registry. Cheaper than a failed',
    '  apply_patch + close-match retry loop. A common failure mode is',
    '  guessing a name like `lfo1gain` from memory when the canonical',
    '  name is `lfo1level` (alias: `lfo1.gain`). When the cheat sheet',
    '  doesn\'t list exactly the param you want, the catalog does.',
    '',
    'Workflow:',
    ...
    'input and produced wrong displays such as 50→39.1%; that\'s fixed.)',
    '',
