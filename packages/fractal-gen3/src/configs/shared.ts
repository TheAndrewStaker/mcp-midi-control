...
export const MODERN_AGENT_GUIDANCE: Readonly<Record<string, string>> = Object.freeze({
  ...
  scene_leveling: [
    'SCENE LEVELING (uneven scene volume is a top pro complaint). The device\'s',
    'own output meters are readable: get_preset with NO location returns',
    'live_meters.output_left/right = momentary output PEAKS at read time',
    '(plus cpu_percent), from the same frame as the live read - no extra',
    'round-trip. These are peak meters, NOT perceived loudness.',
    '',
    'Workflow (the Axe-Edit leveling routine, automated):',
    '1. Per scene: switch_scene N, then have the player play the SAME',
    '   representative passage they will actually use - consistency of what is',
    '   played matters more than what it is. Announce the cadence up front',
    '   ("keep playing until I say switch - each scene takes a few seconds").',
    '2. While they play, call get_preset (live) several times; each read is one',
    '   peak sample. Keep the max (or 95th percentile) of output_left/right',
    '   per scene.',
    '3. Correct on the OUT block: block "output", params OUTPUT_SCENE1..8',
    '   ("Scene 1 Level".."Scene 8 Level") - one param per scene, so writes',
    '   need no scene switch. They are uncalibrated here (raw wire, dB on the',
    '   panel): read the current value first, nudge toward the reference scene,',
    '   re-measure. Iterate rather than compute one exact jump.',
    '4. Pick the LOUDEST scene as reference and trim others toward it (or trim',
    '   the loud one down if the user says it is the outlier).',
    'No auto-save: report the per-scene readings + trims and let the user save.',
    '',
    'HONESTY: momentary peaks are not loudness - a compressed rhythm scene can',
    'peak lower yet sound louder than an open clean. Use the meters to kill the',
    'gross offsets, then have the player A/B by ear for the final call. Meter',
    'decode is community beta (FM9-behaviorally-confirmed; III/FM3 shared path).',
    '',
    'PERCEIVED loudness upgrade: when the unit (or an interface fed by it) is',
    'the computer\'s audio input, measure_loudness returns BS.1770 LUFS per',
    'scene while the player plays - true loudness, not peaks. Same workflow,',
    'better meter; peaks remain the zero-setup fallback.',
    '',
    'PACING (each capture costs the player a played riff): switch_scene + the',
    'announce + the measurement go in ONE turn per scene; after writing trims',
    'run exactly ONE confirm sweep (measure_loudness: use seconds: 5, a',
    'confirm only checks a delta); STOP when scenes agree within ~1.5 LU. Do',
    'not re-sweep unless a trim was re-written after the confirm.',
  ].join('\n'),
  effect_ids: [
