...
export function registerDiscoveryTools(server: McpServer): void {
  server.registerTool('describe_device', {
    description: [
      ...
      'The response also carries `example_spec`: a working apply_preset',
      'payload literal for this device that you can clone instead of',
      'reconstructing the slot / channel / scene shape from prose rules.',
      'Each example uses the device\'s canonical block names and enum',
      'values, uses the right slot shape (bare int for AM4, {row, col}',
      'for grid devices), demonstrates channel-nested vs flat params on',
      'at least one block, and covers at least 2 scenes for devices that',
      'have scenes. Clone the example, swap in the user\'s tone-shape',
      'words, and call apply_preset.',
