...
export function registerDiscoveryTools(server: McpServer): void {
  server.registerTool('describe_device', {
    ...
    description: [
      'REQUIRED first call for any device question or any apply_preset',
      'that uses recipe-style language (auto-wah, parked wah, octave-up,',
      'telephone filter, scene-leveling). Pure introspection, no MIDI I/O,',
      'safe to call repeatedly. Returns the registered device\'s',
      'capabilities and canonical vocabulary.',
      'CHIEF SURFACE: the response carries `recipes`, a per-port list of',
      'frozen knob bundles for common tone vocabulary. Two shapes:',
      'SINGLE-BLOCK recipes (family ∈ auto_wah / pitch / wah / filter /',
      'scene_leveling) carry `target_block` + `params` — paste `params`',
      'into one slot of the chosen `target_block` in apply_preset.',
      'BLOCK-STACK recipes (family = block_stack) carry `slots` — a',
      'multi-block arrangement like `edge_dotted_eighth_lead` (comp +',
      'amp + dotted-8th delay + plate reverb) or `eighties_clean_shimmer`',
      '(clean amp + analog chorus + plate reverb) or `texas_blues_crunch`',
      '(T808 + Plexi-crunch amp + spring reverb). Each block-stack entry',
      'also carries `source_notes` citing where the knob values come',
      'from. Paste the entire `slots` array into apply_preset.spec.slots.',
      'When a user asks for an "Edge-style dotted-8th" / "80s clean" /',
      '"Texas blues" / "auto-wah" / "octave up" / "parked wah" /',
      '"telephone tone" / "scene level balance," scan `recipes[]` for a',
      'matching id BEFORE authoring knob values from scratch. Recipes',
      'compose with custom tweaks: paste the recipe\'s `params` (or',
      '`slots`) into apply_preset, then override individual knobs in the',
      'same call. There is no apply_recipe tool by design; recipes are',
      'reference data, not an indirection layer.',
      'Also tells you what blocks the device has, what its scenes /',
      ...
      'Carries `example_spec`: a working apply_preset payload literal for',
      'this device. Clone it instead of reconstructing slot / channel /',
      'scene shape from prose. Each example uses the device\'s canonical',
      'block names + enum values, the right slot shape (bare int for AM4,',
      '{row, col} for grid devices), and covers at least 2 scenes.',
      'Carries `block_params_summary`: curated top-N first-page knobs',
      'per block in the device\'s canonical spelling (e.g. AM4 reverb ->',
      ...
      'Carries `concept_keys`: cross-device canonical names (e.g.',
      '`drive.output_level`) that rewrite to the device-local name at',
      'dispatch time. Both forms accepted by set_param / apply_preset.',
    ].join(' '),
    inputSchema: {
