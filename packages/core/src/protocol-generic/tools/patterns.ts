...
export function registerPatternTools(server: McpServer): void {
  server.registerTool('apply_pattern', {
    ...
    inputSchema: {
      ...
      pack: z.number().int().min(1).max(32).optional().describe('ncs_upload only (Circuit Tracks): which microSD PACK to write to, numbered as the device shows it, so pack:5 is the front panel\'s "Pack 5". NOTE the two bases in one call: `pack` is 1-based; `ncs_slot` is 0-based. Default pack 1. A card holds up to 32 packs, each a COMPLETE separate world of 64 projects, so the same slot number exists in every pack and the pack decides which project gets overwritten. Call list_packs first to see the card\'s packs by name and pick an empty one. The overwrite gate, the pre-write backup, and the write all target this pack together. IMPORTANT: the server CANNOT detect which pack the device currently has selected (no wire command reports it), so if the user is working in a specific pack, pass it explicitly; otherwise this writes to Pack 1 no matter what the front panel shows. Sample slots (drum_binding, drum_flips, list_samples) are NOT pack-aware yet and always resolve against Pack 1\'s sample pool, so a project written to another pack may point at that pack\'s different samples: verify the kit by ear.'),
  ...
  }, async ({ port, recipe, voices, notation, tab, midi_file, midi_from_bar, midi_bars, midi_tracks, steps, bpm, mode, repeat, armed, ncs_template, ncs_slot, pack, key, transpose, scale, round_robin, drum_flips, drum_binding, confirm_overwrite, backup_first, dry_run, external_targets, arrangement }) => {
    try {
      const result = await executeApplyPattern({
        ...
        bpm: bpm ?? 120,
        mode, repeat, armed, ncs_template, ncs_slot, pack, key, transpose, scale, round_robin, drum_flips, drum_binding, confirm_overwrite, backup_first, dry_run, external_targets, arrangement,
      });
