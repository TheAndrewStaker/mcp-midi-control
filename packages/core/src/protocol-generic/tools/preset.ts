...
export function registerPresetTools(server: McpServer): void {
  ...
  server.registerTool('port_preset', {
    description: [
      ...
      'drive.level via the cross-device alias table), enum-value mapping',
      '(USA IIC+ ↔ USA MK IIC+ via the cross-device concept-key table —',
      '67 high-confidence amp/drive/reverb mappings ship today), scene +',
      'channel cardinality (AM4 4 scenes × 4 channels ↔ II 8 × 2 ↔ III',
      '8 × 4). Modifier wiring is deferred — modifier-driven tones port',
      'as static positions; the user adds the wiring step manually.',
