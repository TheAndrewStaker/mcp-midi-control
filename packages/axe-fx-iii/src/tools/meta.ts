...
export function registerAxeFxIIIMetaTools(server: McpServer): void {
  ...
  server.registerTool('axefx3_reconnect_midi', {
    description: [
      'Drop the cached Axe-Fx III MIDI handle and force a fresh port-open',
      'on the next axefx3_* tool call. Useful when the user plugged the',
      'device in mid-session and the cached "not connected" error keeps',
      'masking the now-working port, or when an earlier tool call timed',
      'out (USB handle may have gone stale).',
      '',
      'This does NOT affect the AM4 / Axe-Fx II / Hydrasynth connections.',
      '',
      BETA_NOTE,
    ].join('\n'),
    inputSchema: {},
  ...
  server.registerTool('axefx3_probe_sysex', {
    description: [
      'Send a raw SysEx byte sequence to the Axe-Fx III AND capture any',
      'inbound MIDI traffic the device emits in response. The community-',
      'capture workflow\'s primary tool: lets testers fire probe bytes at',
      'undecoded function envelopes (STORE_PRESET, SET_GRID_CELL, etc.)',
      'and report the response back to maintainers for offline decode.',
      '',
      'Use this tool for protocol RE — e.g. probing undecoded function',
      'bytes, sweeping payload positions, verifying ACK shapes. NOT for',
      'production preset edits (the III does not yet have a production',
      'set_param tool; this probe is the testing seam).',
      '',
      'WORKFLOW: subscribe to inbound MIDI BEFORE sending (so the',
      'response can\'t race ahead), send the bytes, drain the listener',
      'for `capture_ms` ms, return every observed message with timestamps.',
      'No filtering — every byte the device sent during the window comes',
      'back hex-formatted for inspection.',
      '',
      ...
      '  [+Nms] hex (length)',
      'plus a summary of how many distinct frames arrived. Empty capture',
      'means either (a) the device silently ignored the function byte,',
      '(b) the response arrived after the capture window closed (bump',
      '`capture_ms` and retry), or (c) the input port isn\'t open.',
      '',
      'III-specific: the device auto-emits function 0x21 (FRONT_PANEL_',
      'CHANGE) when the user touches the front panel, and function 0x13',
      '(STATUS_DUMP) on preset/scene load. If your capture window is open',
      'while the user nudges the device, you\'ll see those frames in the',
      'output — useful for confirming auto-emission patterns.',
      '',
      BETA_NOTE,
    ].join('\n'),
    inputSchema: {
