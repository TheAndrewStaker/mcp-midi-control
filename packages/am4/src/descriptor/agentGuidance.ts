...
export const AM4_AGENT_GUIDANCE: Readonly<Record<string, string>> = {
  ...
  loudness: [
    ...
    'change to the user.',
    '',
    'SCENE LEVELING. When you build a multi-scene preset (e.g. Enter Sandman',
    'across 4 scenes), pick ONE scene as the loud reference (usually the',
    'highest-gain rhythm scene) and balance the others within ±3 dB of it',
    'using amp.level (NOT amp.master; master changes tone, level does not).',
    'A clean scene that is more than ~6 dB quieter than the lead scene will',
    'feel disconnected during performance. Conventional spread: clean -3 dB,',
    'crunch 0 dB (reference), lead -1 dB (slight pull-back for clarity),',
    'solo +1 dB.',
    '',
    ...
    'Reverb / "Double Verb Normal" at master=6 = 0 dB). Same field on',
    'drive.type for boost offsets (T808 OD at level=7 = +6 dB reference).',
    'Example: USA MK IIC+ runs ~+6 dB hotter than Double Verb Normal at',
    'the same master/level settings, so a 4-scene preset that uses a clean',
    'Fender voicing for scene 1 and USA MK IIC+ for the lead scene needs',
    'the lead amp.level pulled an extra ~6 dB. Without this correction the',
    'lead scene runs hot and the user hears scenes 2-4 as way louder than',
    'scene 1.',
  ].join('\n'),
  ...
  write_safety_locations: [
    'Any A1..Z4 preset location is accepted for save. Saves to inactive',
    ...
    'A1" without context is suspicious and worth a single-sentence "are',
    'you sure? A1 currently has X" before proceeding. The conventional',
    'scratch slot for try-it-out tone work is "Z4".',
  ].join(' '),
