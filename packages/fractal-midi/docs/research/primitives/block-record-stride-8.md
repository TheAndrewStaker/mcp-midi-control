...
> **SUPERSEDED 2026-07-16 by [[ii-grid-routing-cell-matrix]].** The
> region words 34..129 is the GRID + ROUTING cell matrix: 48
> column-major 2-word cells `(blockId, inputConnectorMask)` starting
> at word 34 (this entry's word-36 origin was one cell off). The
> stride-8 "record" is a grid COLUMN; the flag word is the cell's
> 4-bit input-connection mask (the old "0x0002 = active in standard
> scene" reading is dead: bk070 bypass pairs never touch it); ids >=
> 200 are SHUNTS with real per-cell ids; "ushort[2..7] always zero"
> was an artifact of row-2-only presets (nonzero in 98 multi-row
> presets; preserve-verbatim, never write zeros). Read the new entry;
> this one is retained for the correction trail.
