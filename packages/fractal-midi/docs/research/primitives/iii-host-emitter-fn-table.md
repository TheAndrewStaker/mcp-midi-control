...
- 2026-05-22 (Rosetta-stone primitives audit): promoted to
  primitive. Pre-existing decode work that predated the primitives
  discipline.
...
- 2026-07-09 (BK-054 outer-dispatch mine): re-read of
  `ghidra-axe-edit-iii-host-emitters-precise.txt` found the "~21 fn bytes"
  figure in the Formal definition understates the file's own count (28
  entries with call-site counts). New script
  `FindAxeEditIIIIndirectFnByteCallers.java` decompiled the 2 previously
  un-mined fn=0x00 sites (both hardcode an identical bare-envelope +
  `0x7F`-trailer shape, query-shaped, role undetermined) and ran a broader
  reference scan on the fn=0x08/0x43 "no callers" wrappers, which is a
  DEFINITIVE NEGATIVE for a hidden {name, fnPtr} workflow-registry table:
  the only references found are ordinary PE `.pdata` exception-unwind rows.
  A control check (re-finding the two known fn=0x77 builder call sites via
  the same broadened scan) PASSED, so the negative is trusted, not a stale-
  pointer artifact; full findings in
  `packages/fractal-midi/docs/devices/axe-fx-iii/SYSEX-MAP.md` "BK-054"
  section. Separately, the async-workflow registry
  ([[iii-async-workflow-fn-registry]]) was re-grepped in full for any
  DSP/CPU/meter-named workflow (BK-055 ask): zero matches among ~140 rows,
  closing that lane too; see [[_negative/iii-sysex-dsp-message-unbound]].
