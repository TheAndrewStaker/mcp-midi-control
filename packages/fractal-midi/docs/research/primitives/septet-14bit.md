...
golden: scripts/primitives-verify.ts#case-septet-14bit
...
consumed_in:
  ...
  - fractal-midi/samples/captured/decoded/ghidra-axe-edit-iii-actions-and-shapes.txt (5-septet 32-bit width-variant at FUN_14033ec70 L22777-22781 and FUN_140336a40 L23071-23075; N=2 inside the III binary confirms the same shift-table {0, 7, 14, 21, 28} this primitive documents for the 2-septet form generalizes to 5 septets. The FUN_140336a40 site spells the 28-shift as `>> 0x18 >> 4` and emits via `FUN_1403437d0(.., 0x5c, buf, 5, ..)`. Line numbers are regeneration-sensitive: the function names are the stable anchor, so re-find by `--- FUN_x @ ---` if a re-dump shifts them, as the 2026-07-15 regeneration did by 5 lines)
---
...
`scripts/primitives-verify.ts#case-septet-14bit` runs round-trip fixtures
