...
## Discrete ordinal-in-word (2026-07-16 refinement)

The word at `paramBase + paramId` for a ROSTERED SELECT param holds
the raw wire ordinal itself, no calibration: 24,082+ rostered-select
X-channel observations across the corpus (384 Q8.02 factory + live
hardware dumps + the 2026-07-10 front-panel-confirmed fixture) decode
in-roster with zero sentinels and zero out-of-roster values (tempo
dual-mode selects and bypass-pid housekeeping words excluded by name).
Three independent oracles agree: the device fn 0x28 label dump
([[fn28-enum-dump]], 154/155 vs catalog), the hardware fixture (drive
X word 844 = 6 = T808 OD, Y word 865 = 36 = BLACKGLASS 7K), and the
BK-070 amp-type-ordinal-0 anchor at word 149 on A000. Byte-2 reserved
bits are ZERO on every select word in the corpus (61k+ observations).
The existing word-patch mechanism therefore expresses discrete values
exactly; the Y-channel hazard gates (Multi Delay / Controllers never;
stale amp-Y read-before-write) live in the encoder:
`packages/fractal-midi/src/gen2/axe-fx-ii/presetImage/discretePatch.ts`,
golden `scripts/verify-ii-image-discrete-encode.ts`.
...
- 2026-05-22 (primitives audit): refreshed the entry to reflect Session
...
- 2026-07-16: discrete ordinal-in-word section added (corpus census +
  fn 0x28 + hardware-fixture oracles); discrete word-patch encoder
  shipped in fractal-midi with the Y-hazard gates; second golden
  scripts/verify-ii-image-discrete-encode.ts (substrate identity,
  census, donor-splice, fixture anchors).
