...
| [gen3-signed-int16-semitone-register](gen3-signed-int16-semitone-register.md) | value-encoding | matched | FM9 / III | PITCH/PLEX/SYNTH/REVERB "shift" params (-24..24 semitones): WRITE is the standard normalized-continuous form; READ returns a raw signed int16 register, un-renormalized (wire 65512 = -24, not the standard-formula +24). Asymmetric encode/decode. 32/32 params confirmed on FM9 fw 11.0 + III fw 25.04 hardware roundtrip (issue #6) |
...
| [deferred-commit-readback-window](deferred-commit-readback-window.md) | verification-method | partial-N1 | any (measured on Circuit Tracks) | A device that commits stored state ASYNCHRONOUSLY has a window in which its own listing lags its own flash, so an early read-back reports a good write as absent (Circuit: manifest flushed ~6-8 s AFTER session CLOSE; a 1.2 s read called 8 slots empty that were all present later). Rule: presence is conclusive whenever seen, ABSENCE is conclusive only after the window; poll past it. Cost is asymmetric, a false "write failed" triggers a redo, and on a device with no erase a redo is not harmless. Distinct from the refuted in-session commit-WAIT: the write does not need the wait, the reader does |
| [device-reserialization-oracle](device-reserialization-oracle.md) | verification-method | partial-N1 | any (worked on Circuit Tracks) | Writing a value into a stored file and reading it back proves only that a FILE round-trips it. Make the device re-author the file from its own state, and watch an UNRELATED field that can only be composed from live hardware (a physical control position) move on its own: that field is the oracle. Asymmetry: a CHANGED value under test is conclusive with no oracle; the oracle is only needed when the value SURVIVES. Limit: proves the ORACLE's region was rebuilt, the field under test is a strong inference from the same save path, not a measurement |
