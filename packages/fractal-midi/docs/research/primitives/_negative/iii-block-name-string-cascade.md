...
class: struct-layout
status: non-matching
discovered:  (primitives transfer-candidate audit)
verified_on:
  - axe-edit-iii-1.40
firmware_sensitive: false
golden: scripts/primitives-verify.ts#case-iii-block-name-string-cascade
retest_when:
  - regenerated III preset-related Ghidra dumps (case-iii-block-name-string-cascade re-adjudicates the strcmp grep automatically on every primitives-verify run)
...
mid=8, byte_count=3072)` (primitive entry
...
## How this fits in the primitives corpus
...
absent from the editor's serialization code path. The primitives reader
...
- 2026-05-22: negative finding committed after
  ...
  session primitives discipline per CLAUDE.md "negative findings are
  valuable" section.
