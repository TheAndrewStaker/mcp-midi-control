...
(`src/am4/bodyChain.ts`). Primitive `[[am4-gen3-preset-container]]`.
...
`scripts/verify-ii-preset-image-tlv.ts`; SYSEX-MAP § 6b; primitive
...
field 0x08/0x09 = target effectId/paramId, 0x00 = source). Primitive
...
| 7 | Make the primitives-verify golden pointer load-bearing (warn when a non-STUB pointer names a missing fixture key) | `scripts/primitives-verify.ts` | Yes | Prevents silent drift between frontmatter golden pointers and the actual fixture table |
