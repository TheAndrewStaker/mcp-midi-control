...
## Section 1. Instances of existing primitives

### 1.1 [[vendor-envelope-descriptor-table]], fn=0x40 LOAD_PRESET descriptor at `0x1407ab2f0`

**Location.** `FUN_140337060` @ L51-83. The function walks the table with 12-byte stride (`0xc`) and `-1` sentinel, exactly the primitives corpus shape.
...
**Proposed primitives update.** Add to `vendor-envelope-descriptor-table.md`'s `consumed_in:` list:

```
- fractal-midi/samples/captured/decoded/ghidra-axe-edit-iii-store-preset.txt (FUN_140337060 walks 0x1407ab2f0; L51-83)
```

`0x1407ab2f0` is already mentioned in `xor-fold-hash.md` (as the III store-preset descriptor candidate) but the current primitives corpus body for `vendor-envelope-descriptor-table` doesn't list it explicitly. The table is in-range of the catalog's `0x1407aac70..0x1407abb60` summary but the per-table breakdown skips it. Promoting it from the existing fn-byte-envelopes-ghidra.md catalog into the primitives corpus body is a small refinement.
...
**Proposed primitives update.** Add to `msb-first-14bit-preset-payload.md`'s `consumed_in:` list:
...
**Proposed primitives update.** Add to `septet-14bit.md`'s `consumed_in:` list:
...
**Proposed primitives update.** Add to `iii-host-emitter-fn-table.md`'s `consumed_in:` list:
...
discovered: 2026-05-22 (primitive mining of ghidra-axe-edit-iii-store-preset.txt)
verified_on:
  - axe-edit-iii-binary
firmware_sensitive: false
golden: scripts/primitives-verify.ts#case-iii-byte-stream-septet-pack-8to7  # TBD
...
**Why it matters.** The existing primitive entry [[iii-fn01-set-parameter-envelope]] documents the SET sub-action specifically (status `matched`) but the underlying builder is broader. Promoting the 6-field structure into the primitive entry's "Formal definition" section means:
- Future decoders for the STATE_BROADCAST and long-broadcast envelopes can cite the same primitive.
- The Field D (32-bit IEEE-754 float) ambiguity vs the primitives corpus's "value=508" fixture gets clarified.
...
### 2.3 `iii-stream-patch-preset-index`: workflow pattern, NOT recommended for primitives promotion
...
**Proposed negative primitive entry.**
...
Primitives discipline: when a 1-byte fn-byte envelope's caller allocates
...
**Hypothesis flagged for primitives clarification.** The existing [[iii-fn01-set-parameter-envelope]] verification fixture reads `value=508` for Drive 1 Boost ON. The dump confirms via `FUN_14033ec70` that Field D is a 32-bit LSB-first 5-septet field whose bytes `00 00 00 7C 03` decode to `0x3F800000` = IEEE-754 float `1.0`, not integer `508`.

The two readings coincide for the SET_PARAMETER capture corpus because all four captured values (boost ON, boost OFF, time 520, time 516) sit in the low 14 bits of Field D; the high 21 bits are always zero, so the bytes look identical under integer-vs-float interpretation. But a consumer that builds a new write with `value=1000000` per the primitive fixture's integer interpretation will pack `0x40 0x42 0x0F 0x00 0x00` (1000000 as LSB-first 5-septet integer) into Field D, while the device firmware likely expects `value=1000000.0` packed as IEEE-754 float bits `0x49742400`. These produce DIFFERENT 5-byte sequences, and the wire write will be silently incorrect.

**This is not a primitive bug per se**: the entry says "5-byte septet-encoded packed-float per the AM4-derived `packValue` algorithm." But the explicit `value=508` in the fixture text is ambiguous given the float interpretation. A clarifying refinement to the Verification path section, naming Field D's contents as `float-bits(value)` not raw `value`, would prevent a future misuse.

I'm not proposing this as a `_negative` entry (the primitive entry is not wrong, just under-specified). Recommended instead: refinement-history note on `iii-fn01-set-parameter-envelope.md`.

## Summary of proposed primitives actions
...
All seven actions are founder-gated. No primitives files modified by this report.
...
| `FUN_14033f2d0` | L1278-1317 | General 8-to-7-bit byte-stream septet packer. Output size = `ceil(N*8/7) + 1`. Candidate new primitive (§2.1). |
