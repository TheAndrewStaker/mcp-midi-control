/**
 * Boss RC-family `.RC0` memory-file codec. Structure-preserving parse +
 * byte-exact serialize + path-addressable surgical leaf edit.
 *
 * VALIDATED byte-exact against real device files (2026-07-07): every RC-505mk2
 * memory (287,784 B / 20,864 leaves), both `SYSTEM*.RC0`, and `RHYTHM.RC0`
 * round-trip byte-for-byte, plus the full sibling corpus (RC-600 / RC-505 mk1 /
 * RC-500). See docs/manuals/other-gear/RC-505mk2-RC0-SCHEMA.md.
 *
 ...
 * envelope, and an opaque trailing region after `</database>` (RC-600 / mk2
 * carry `<count>HHHH</count>` with NO final newline; mk1 / RC-500 end at
 ...
 * structural and dialect-agnostic; the only per-model knowledge is the
 * tag-letter dictionary for AUTHORING specific fields (see mk2.ts), which
 * addresses leaves by PATH so read-modify-write preserves every other byte
 * (the same byte-exact discipline as the SPD-SX `.spd` codec).
 *
 * Files are read/written byte-preserving via latin1 (names are stored as
 * integer char CODES, so there is no multibyte concern). A binary `RHYTHM.RC0`
 * (a `PTN_0000` header, not XML) round-trips untouched and indexes 0 leaves.
 */
 ...
 * space(32)/NUL(0)-padded field trims cleanly.
 */
