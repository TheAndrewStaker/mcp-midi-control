 ...
 *
 * The family generalizes like Fractal gen-3: one codec (`codec/rc0` + the
 * per-model dictionary in `codec/mk2`) bound to per-model CONFIGS. The mk2 config
 * ships first; the RC-600 (shares the A/B storage shape) and the RC-500 / mk1
 * (single-file, no `<count>`) are later configs with their own dictionaries.
 */
...
export interface BossRcConfig {
   ...
   * not readable over MIDI), so it is overridable at runtime via the
   * `MCP_RC505_CTL_CHANNEL` env var.
   */
...
const CTL_CHANNEL_ENV = 'MCP_RC505_CTL_CHANNEL';
...
function resolveCtlChannel(config: BossRcConfig): number {
  const raw = process.env[CTL_CHANNEL_ENV];
  if (raw && raw.trim() !== '') {
...
const ASSIGN_BLOCK: BlockSchema = {
...
const BOSS_RC_BLOCKS: Readonly<Record<string, BlockSchema>> = {
  assign: ASSIGN_BLOCK,
...
function bossRcAgentGuidance(config: BossRcConfig): Readonly<Record<string, string>> {
  ...
  return Object.freeze({
    ...
    live_control:
      ...
      `(default ch${ch}; set MCP_RC505_CTL_CHANNEL to the unit's actual RX CTL channel). Looper/track functions: ` +
...
export function createBossRcDescriptor(config: BossRcConfig): DeviceDescriptor {
  ...
  return {
    ...
    canonical_terms: {
      ...
      preset: 'memory',
      scene: 'n/a (the RC has no scenes; its 5 tracks are the performance layers)',
      channel: 'RX CTL channel (the memory-recall + control channel)',
      location: 'memory (1-99)',
    },
    blocks: BOSS_RC_BLOCKS, // one "block" = one ASSIGN control-routing slot (see BOSS_RC_BLOCKS)
    reader: {
      async getParam() {
      ...
      async getPreset(ctx, options?: GetPresetOptions): Promise<PresetSnapshot> {
        ...
        const id = memoryIdIn(active.doc) ?? '0';
        const assigns = readAllAssigns(active.doc, id);
        return {
      ...
      async switchPreset(ctx, location: LocationRef): Promise<WriteResult> {
        ...
        return {
          ...
          info:
            ...
            'Requires PC IN = ON and the unit\'s RX CTL channel to match (set MCP_RC505_CTL_CHANNEL if it is not ' +
